...
function parseArgs(rawArgs) {
    ...
    return {
      ...
      url: args['-u'] || false,
      inputArg: args['-u'] || args['-j'] || args['-g'] || args['-b'] || args['-i'] || args['-t'] ? rawArgs.slice(3) : rawArgs.slice(2),
...
export function cli(args) {
    ...
    if ((!parseArgs.version && !parseArgs.url && parsedArgs.inputArg == 0) || (parseArgs.version && parseArgs.url)){
        ...
        .then(body => linkCheck(body, parsedArgs.jsonOutput, parsedArgs.showGood, parsedArgs.showBad, parsedArgs.telescopeJsonLinkTest))
        .catch((error) => {
